...
export default async function handler(req: VercelRequest, res: VercelResponse) {
    ...
    try {
        ...
        if (!SERVICE_KEY && !ANON_KEY) missing.push('SUPABASE_SERVICE_ROLE_KEY o VITE_SUPABASE_ANON_KEY');
        ...
        if (missing.length > 0) {
            return res.status(200).json({ 
                success: false, 
                error: `Faltan variables en Vercel: ${missing.join(', ')}. Agrégalas en el panel de project settings.` 
            });
        }
